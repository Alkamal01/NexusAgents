...
from nexus_agents.const import DEFAULT_WORKSPACE_ROOT, TEST_DATA_PATH
from nexus_agents.context import Context as MetagptContext
...
def llm_mock(rsp_cache, mocker, request):
    ...
    mocker.patch("nexus_agents.provider.base_llm.BaseLLM.aask", llm.aask)
    mocker.patch("nexus_agents.provider.base_llm.BaseLLM.aask_batch", llm.aask_batch)
    mocker.patch("nexus_agents.provider.openai_api.OpenAILLM.aask_code", llm.aask_code)
    yield mocker
...
def new_filename(mocker):
    ...
    mocker.patch("nexus_agents.utils.file_repository.FileRepository.new_filename", lambda: "20240101")
    yield mocker
