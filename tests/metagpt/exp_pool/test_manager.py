...
from nexus_agents.configs.exp_pool_config import (
    ExperiencePoolConfig,
    ExperiencePoolRetrievalType,
)
from nexus_agents.configs.llm_config import LLMConfig
from nexus_agents.exp_pool.manager import Experience, ExperienceManager
from nexus_agents.exp_pool.schema import DEFAULT_SIMILARITY_TOP_K, QueryType
...
class TestExperienceManager:
    ...
    def test_create_bm25_storage(self, mocker, mock_config):
        mocker.patch("nexus_agents.rag.engines.SimpleEngine.from_objs", return_value=mocker.MagicMock())
        mocker.patch("nexus_agents.rag.engines.SimpleEngine.from_index", return_value=mocker.MagicMock())
        mocker.patch("nexus_agents.rag.engines.SimpleEngine.get_obj_nodes", return_value=[])
        mocker.patch("nexus_agents.rag.engines.SimpleEngine._resolve_embed_model", return_value=mocker.MagicMock())
        mocker.patch("llama_index.core.VectorStoreIndex", return_value=mocker.MagicMock())
        mocker.patch("nexus_agents.rag.schema.BM25RetrieverConfig", return_value=mocker.MagicMock())
