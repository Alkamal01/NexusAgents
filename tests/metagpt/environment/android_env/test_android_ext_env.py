...
from nexus_agents.environment.android.android_ext_env import AndroidExtEnv
from nexus_agents.environment.android.const import ADB_EXEC_FAIL
...
def test_android_ext_env(mocker):
    ...
    mocker.patch("nexus_agents.environment.android.android_ext_env.AndroidExtEnv.execute_adb_with_cmd", mock_device_shape)
    mocker.patch("nexus_agents.environment.android.android_ext_env.AndroidExtEnv.list_devices", mock_list_devices)
    ...
    mocker.patch(
        "nexus_agents.environment.android.android_ext_env.AndroidExtEnv.execute_adb_with_cmd", mock_device_shape_invalid
    )
    assert ext_env.device_shape == (0, 0)

    assert ext_env.list_devices() == [device_id]

    mocker.patch("nexus_agents.environment.android.android_ext_env.AndroidExtEnv.execute_adb_with_cmd", mock_get_screenshot)
    assert ext_env.get_screenshot("screenshot_xxxx-xx-xx", "/data/") == Path("/data/screenshot_xxxx-xx-xx.png")

    mocker.patch("nexus_agents.environment.android.android_ext_env.AndroidExtEnv.execute_adb_with_cmd", mock_get_xml)
    assert ext_env.get_xml("xml_xxxx-xx-xx", "/data/") == Path("/data/xml_xxxx-xx-xx.xml")

    mocker.patch(
        "nexus_agents.environment.android.android_ext_env.AndroidExtEnv.execute_adb_with_cmd", mock_write_read_operation
    )
    res = "OK"
