...
from nexus_agents.actions import RunCode
from nexus_agents.const import get_metagpt_root
from nexus_agents.roles.tutorial_assistant import TutorialAssistant
from nexus_agents.schema import Message
from nexus_agents.utils.common import (
    NoMoneyException,
    OutputParser,
    any_to_str,
    any_to_str_set,
...
class TestGetProjectRoot:
    ...
    def test_any_to_str(self):
        ...
        inputs = [
            Input(x=TutorialAssistant, want="nexus_agents.roles.tutorial_assistant.TutorialAssistant"),
            Input(x=TutorialAssistant(), want="nexus_agents.roles.tutorial_assistant.TutorialAssistant"),
            Input(x=RunCode, want="nexus_agents.actions.run_code.RunCode"),
            Input(x=RunCode(), want="nexus_agents.actions.run_code.RunCode"),
            Input(x=Message, want="nexus_agents.schema.Message"),
            Input(x=Message(content=""), want="nexus_agents.schema.Message"),
            Input(x="A", want="A"),
        ]
        for i in inputs:
            v = any_to_str(i.x)
    ...
    def test_any_to_str_set(self):
        ...
        inputs = [
            Input(
                ...
                want={"nexus_agents.roles.tutorial_assistant.TutorialAssistant", "nexus_agents.actions.run_code.RunCode", "a"},
            ),
            Input(
                x={TutorialAssistant, "a"},
                want={"nexus_agents.roles.tutorial_assistant.TutorialAssistant", "a"},
            ),
            Input(
                x=(TutorialAssistant, RunCode(), "a"),
                want={"nexus_agents.roles.tutorial_assistant.TutorialAssistant", "nexus_agents.actions.run_code.RunCode", "a"},
            ...
            Input(
                ...
                want={"a", "nexus_agents.roles.tutorial_assistant.TutorialAssistant", "nexus_agents.actions.run_code.RunCode"},
            ),
        ]
        for i in inputs:
    ...
    @pytest.mark.parametrize("module_path", ["tests.nexus_agents.utils.test_common"])
    ...
    def test_import_class_inst(self):
        rc = import_class_inst("RunCode", "nexus_agents.actions.run_code", name="X")
        assert rc.name == "X"
