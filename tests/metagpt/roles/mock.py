...
from nexus_agents.actions import UserRequirement, WriteDesign, WritePRD, WriteTasks
...
资源：
1. 大语言模型已经有前置的抽象、部署，可以通过 `from nexus_agents.llm import LLM`，再使用`LLM().ask(prompt)`直接调用
