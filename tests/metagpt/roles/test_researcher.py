...
from nexus_agents.actions.research import CollectLinks
from nexus_agents.roles import researcher
...
async def test_researcher(mocker, search_engine_mocker, context):
    with TemporaryDirectory() as dirname:
        ...
        mocker.patch("nexus_agents.provider.base_llm.BaseLLM.aask", mock_llm_ask)
...
def test_write_report(mocker, context):
    with TemporaryDirectory() as dirname:
        for i, topic in enumerate(
            ...
            assert (researcher.RESEARCH_PATH / f"{i+1}. nexus_agents.md").read_text().startswith("# Research Report")
