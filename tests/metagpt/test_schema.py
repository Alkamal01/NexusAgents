...
from nexus_agents.const import SERDESER_PATH, SYSTEM_DESIGN_FILE_REPO, TASK_FILE_REPO
...
def test_message_serdeser():
    ...
    assert message_dict["cause_by"] == "nexus_agents.actions.write_code.WriteCode"
    ...
    assert new_message.cause_by == "nexus_agents.actions.add_requirement.UserRequirement"
...
class TestSerializationMixin:
    ...
    def mock_write_json_file(self, mocker):
        return mocker.patch("nexus_agents.schema.write_json_file")

    @pytest.fixture
    def mock_read_json_file(self, mocker):
        return mocker.patch("nexus_agents.schema.read_json_file")
    ...
    def test_serialize_with_exclude(self, mock_write_json_file):
        ...
        expected_data = {
            "name": "test",
            "value": 42,
            "__module_class_name": "tests.nexus_agents.test_schema.TestUserModelWithExclude",
        }
