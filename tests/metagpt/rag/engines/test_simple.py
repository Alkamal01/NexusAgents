...
from nexus_agents.rag.engines import SimpleEngine
from nexus_agents.rag.parsers import OmniParse
from nexus_agents.rag.retrievers import SimpleHybridRetriever
from nexus_agents.rag.retrievers.base import ModifiableRAGRetriever, PersistableRAGRetriever
from nexus_agents.rag.schema import BM25RetrieverConfig, ObjectNode
...
class TestSimpleEngine:
    ...
    def mock_simple_directory_reader(self, mocker):
        return mocker.patch("nexus_agents.rag.engines.simple.SimpleDirectoryReader")

    @pytest.fixture
    def mock_get_retriever(self, mocker):
        return mocker.patch("nexus_agents.rag.engines.simple.get_retriever")

    @pytest.fixture
    def mock_get_rankers(self, mocker):
        return mocker.patch("nexus_agents.rag.engines.simple.get_rankers")

    @pytest.fixture
    def mock_get_response_synthesizer(self, mocker):
        return mocker.patch("nexus_agents.rag.engines.simple.get_response_synthesizer")

    @pytest.fixture
    def mock_get_file_extractor(self, mocker):
        return mocker.patch("nexus_agents.rag.engines.simple.SimpleEngine._get_file_extractor")
    ...
    def test_from_index(self, mocker, mock_llm, mock_embedding):
        ...
        mock_get_index = mocker.patch("nexus_agents.rag.engines.simple.get_index")
    ...
    async def test_aretrieve(self, mocker):
        # Mock
        mock_query_bundle = mocker.patch("nexus_agents.rag.engines.simple.QueryBundle", return_value="query_bundle")
        mock_super_aretrieve = mocker.patch(
            "nexus_agents.rag.engines.simple.RetrieverQueryEngine.aretrieve", new_callable=mocker.AsyncMock
        )
    ...
    def test_add_docs(self, mocker):
        # Mock
        mock_simple_directory_reader = mocker.patch("nexus_agents.rag.engines.simple.SimpleDirectoryReader")
        mock_simple_directory_reader.return_value.load_data.return_value = [
            Document(text="document1"),
            Document(text="document2"),
        ]
        ...
        mock_run_transformations = mocker.patch("nexus_agents.rag.engines.simple.run_transformations")
    ...
    def test_with_obj_metadata(self, mocker):
        ...
        mock_import_class = mocker.patch("nexus_agents.rag.engines.simple.import_class")
    ...
    def test_get_file_extractor(self, mocker):
        ...
        mock_omniparse_config = mocker.patch("nexus_agents.rag.engines.simple.config.omniparse", autospec=True)
