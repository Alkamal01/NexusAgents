...
from nexus_agents.rag.factories.index import RAGIndexFactory
...
class TestRAGIndexFactory:
    ...
    def mock_storage_context(self, mocker):
        return mocker.patch("nexus_agents.rag.factories.index.StorageContext.from_defaults")

    @pytest.fixture
    def mock_load_index_from_storage(self, mocker):
        return mocker.patch("nexus_agents.rag.factories.index.load_index_from_storage")

    @pytest.fixture
    def mock_from_vector_store(self, mocker):
        return mocker.patch("nexus_agents.rag.factories.index.VectorStoreIndex.from_vector_store")
    ...
    def test_create_faiss_index(
        ...
        # Mock
        mock_faiss_store = mocker.patch("nexus_agents.rag.factories.index.FaissVectorStore.from_persist_dir")
    ...
    def test_create_chroma_index(self, mocker, chroma_config, mock_from_vector_store, mock_embedding):
        # Mock
        mock_chroma_db = mocker.patch("nexus_agents.rag.factories.index.chromadb.PersistentClient")
        mock_chroma_db.get_or_create_collection.return_value = mocker.MagicMock()

        mock_chroma_store = mocker.patch("nexus_agents.rag.factories.index.ChromaVectorStore")
    ...
    def test_create_es_index(self, mocker, es_config, mock_from_vector_store, mock_embedding):
        # Mock
        mock_es_store = mocker.patch("nexus_agents.rag.factories.index.ElasticsearchStore")
