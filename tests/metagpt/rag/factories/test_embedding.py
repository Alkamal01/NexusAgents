...
from nexus_agents.configs.embedding_config import EmbeddingType
from nexus_agents.configs.llm_config import LLMType
from nexus_agents.rag.factories.embedding import RAGEmbeddingFactory
...
class TestRAGEmbeddingFactory:
    ...
    def mock_openai_embedding(mocker):
        return mocker.patch("nexus_agents.rag.factories.embedding.OpenAIEmbedding")

    @staticmethod
    def mock_azure_embedding(mocker):
        return mocker.patch("nexus_agents.rag.factories.embedding.AzureOpenAIEmbedding")

    @staticmethod
    def mock_gemini_embedding(mocker):
        return mocker.patch("nexus_agents.rag.factories.embedding.GeminiEmbedding")

    @staticmethod
    def mock_ollama_embedding(mocker):
        return mocker.patch("nexus_agents.rag.factories.embedding.OllamaEmbedding")
