...
from nexus_agents.const import LLM_API_TIMEOUT
...
class MockLLM(OriginalLLM):
    ...
    async def original_aask_batch(self, msgs: list, timeout=LLM_API_TIMEOUT) -> str:
        """A copy of nexus_agents.provider.base_llm.BaseLLM.aask_batch, we can't use super().aask because it will be mocked"""
        context = []
        for msg in msgs:
    ...
    async def original_aask_code(self, messages: Union[str, Message, list[dict]], **kwargs) -> dict:
        """
        A copy of nexus_agents.provider.openai_api.OpenAILLM.aask_code, we can't use super().aask because it will be mocked.
