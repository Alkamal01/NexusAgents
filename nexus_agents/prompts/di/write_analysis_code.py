...
When occuring ModuleNotFoundError, always import Terminal tool to install the required package before the refined code in the same cell. Such as `from nexus_agents.tools.libs.terminal import Terminal\nterminal = Terminal()\nawait terminal.run_command('pip install pandas')` before importing pandas.
"""
...
```python
from nexus_agents.tools.libs.data_preprocess import get_column_info
