...
from nexus_agents.base.base_serialization import BaseSerialization
from nexus_agents.const import (
    AGENT,
...
from nexus_agents.repo_parser import DotClassInfo
...
from nexus_agents.utils.exceptions import handle_exception
from nexus_agents.utils.report import TaskReporter
from nexus_agents.utils.serialize import (
...
class Message(BaseModel):
    ...
    def check_instruct_content(cls, ic: Any) -> BaseModel:
        if ic and isinstance(ic, dict) and "class" in ic:
            if "mapping" in ic:
                ...
                actionnode_class = import_class("ActionNode", "nexus_agents.actions.action_node")  # avoid circular import
    ...
    def check_cause_by(cls, cause_by: Any) -> str:
        return any_to_str(cause_by if cause_by else import_class("UserRequirement", "nexus_agents.actions.add_requirement"))
    ...
    def ser_instruct_content(self, ic: BaseModel) -> Union[dict, None]:
        ...
        if ic:
            ...
            ic_type = str(type(ic))
            if "<class 'nexus_agents.actions.action_node" in ic_type:
